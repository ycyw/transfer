...
注：若搭建完成后无法使用，查看服务器的安全组/防火墙是否放开了相应端口，教程使用端口为6666，可以改成自己想用的端口，1024-49151任选一个端口号即可（教程使用的腾讯云CVM服务器默认放开全部端口，不需要配置）。若端口已开放但连接失败，使用矿机ping服务器公网IP，服务器ping矿池地址，查看是否ping通，若有一边ping失败，腾讯云CVM服务器可更换IP地址后使用新IP再连接  
## 修改端口转发地址  
#### 1.查看当前监听端口的id  
```
